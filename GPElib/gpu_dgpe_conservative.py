...
class DGPE_ODE(torch.nn.Module):
	...
	def __init__(self, device, N_wells, J, anisotropy, gamma,
		...
		self.J = torch.nn.Parameter(torch.tensor(np.zeros(N_wells) + J).to(device))
		self.anisotropy = torch.nn.Parameter(torch.tensor(np.zeros(N_wells) + anisotropy).to(device))

		self.gamma = torch.nn.Parameter(torch.tensor(np.zeros(N_wells) + gamma).to(device))

		# self.torch_N_wells = torch.tensor(self.N_wells, tf.int64)

		self.nn_idx_1 = torch.nn.Parameter(torch.tensor(nn_idx_1, dtype=torch.int64).to(device))
		self.nn_idx_2 = torch.nn.Parameter(torch.tensor(nn_idx_2, dtype=torch.int64).to(device))
		self.nn_idy_1 = torch.nn.Parameter(torch.tensor(nn_idy_1, dtype=torch.int64).to(device))
		self.nn_idy_2 = torch.nn.Parameter(torch.tensor(nn_idy_2, dtype=torch.int64).to(device))
		self.nn_idz_1 = torch.nn.Parameter(torch.tensor(nn_idz_1, dtype=torch.int64).to(device))
		self.nn_idz_2 = torch.nn.Parameter(torch.tensor(nn_idz_2, dtype=torch.int64).to(device))
		...
		self.N_wells = torch.nn.Parameter(torch.tensor(N_wells, dtype=torch.int64).to(device))
		...
		self.h_dis_x_flat = torch.nn.Parameter(torch.tensor(h_dis_x_flat).to(device))
		self.h_dis_y_flat = torch.nn.Parameter(torch.tensor(h_dis_y_flat).to(device))

		self.beta_disorder_array_flattened = torch.nn.Parameter(torch.tensor(beta_disorder_array_flattened).to(device))
		self.beta = torch.nn.Parameter(torch.tensor(beta_flat).to(device))
		self.e_disorder = torch.nn.Parameter(torch.tensor(e_disorder_flat).to(device))
